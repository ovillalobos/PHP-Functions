...
if ( $user )
{
    $t = new eZTemplate( "eztransaccion/user/" . $ini->read_var( "eZTransaccionMain", "TemplateDir" ),
                         "eztransaccion/user/intl/", $Language, "dispersion.php" );
...
//HB   
    ...
    case "FrAc":
        $particularFields = "";
        $tr->blog($qki,"Dispersion", $_POST['FrAccount'], $_POST['ToAccount'], $DiasPzo, $_POST['Amount'], $Accion);
        break;
    case "Confirm":
    case "Process":
        $particularFields = "&Passwd=".encrypt( $code, strtolower( $usr ) )."&Day=".urlencode($_POST['Day'])."&Month=".urlencode($_POST['Month'])."&FrAccount=".urlencode($_POST['FrAccount'])."&ToAccount=".urlencode($_POST['ToAccount'])."&Amount=".urlencode($_POST['Amount'])."&Desc=".urlencode(preg_replace("/&/", "-", $_POST['Desc'])); // DBA 13Jul2007 Solicitar token en concentración de tesoreria empresarial   //HB AGL
        //$particularFields = &Day=".urlencode($_POST['Day'])."&Month=".urlencode($_POST['Month'])."&FrAccount=".urlencode($_POST['FrAccount'])."&ToAccount=".urlencode($_POST['ToAccount'])."&Amount=".urlencode($_POST['Amount'])."&Desc=".urlencode(ereg_replace("&", "-", $_POST['Desc'])); // DBA 13Jul2007 Solicitar token en concentración de tesoreria empresarial
        $tr->blog($qki,"Dispersion", $_POST['FrAccount'], $_POST['ToAccount'], $DiasPzo, $_POST['Amount'], $Accion);
        break;
    }
    $transaccion_buffer = "";
    // DebugBreak();
    $ret_code = $tr->PostToHost($backend, "/IBnkIIS.dll", "Trxn=dis&Access=".urlencode($_POST['Access'])."&CustID=".urlencode($usr)."&Cadpriv=".urlencode($priv).$particularFields, $qki, $usr, $qki, $priv, $transaccion_buffer); // dispersion de fondos
	...
	if ($_POST['Access'] == "Process" and strpos($transaccion_buffer,"No. de Autoriza") != false)
	{
		...
		$Pagina = $Pagina."</P></CENTER>";
		$Pagina = $Pagina."<H2>Dispersión</H2>";
		...
		/*$FechaHora = $date . " " . $time;
		$QryRIC = new eZReImp( );
		$QryRIC -> store($_POST['CustID'],$_POST['FrAccount'],$buffer2,"dis","Dispersi&oacute;n",$FechaHora);*/
		//DBA Reimpresion de Comprobantes
	}
    ...
    $t->pparse( "output", "dispersion_tpl" );
}
else
{
    $t = new eZTemplate( "eztransaccion/user/" . $ini->read_var( "eZTransaccionMain", "TemplateDir" ),
    ...
    $t->set_var( "redirect_url", "/transaccion/dispersion/" );
