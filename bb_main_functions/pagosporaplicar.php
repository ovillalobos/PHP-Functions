...
if ( $user )
{
    ...
    $t = new eZTemplate( "eztransaccion/user/" . $ini->read_var( "eZTransaccionMain", "TemplateDir" ),
                         "eztransaccion/user/intl/", $Language, "PagosPorAplicar.php" );

    $t->setAllStrings();

    $t->set_file( array(
        "PagosPorAplicar_tpl" => "PagosPorAplicar.tpl"
        ) );
    ...
    $t->pparse( "output", "PagosPorAplicar_tpl" );

    $usr = strtolower($usr);   

    $result = ControlAcceso("canpag",$usr);

    if($result=="0000")
    	include("eztransaccion/user/include/pagosporaplicar.inc");
    else
        echo $result;
