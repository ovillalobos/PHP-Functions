...
if ( $user )
{
    ...
    $t = new eZTemplate( "eztransaccion/user/" . $ini->read_var( "eZTransaccionMain", "TemplateDir" ),
                         "eztransaccion/user/intl/", $Language, "CapturaPagos.php" );

    $t->setAllStrings();

    $t->set_file( array(
        "CapturaPagos_tpl" => "CapturaPagos.tpl"
        ) );
    ...
    $t->pparse( "output", "CapturaPagos_tpl" );

    $usr = strtolower($usr);


    $result = ControlAcceso("cappag",$usr);

    if($result=="0000")
    	include("eztransaccion/user/include/capturapagos.inc");
    else
        echo $result;
