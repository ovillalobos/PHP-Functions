...
//echo "[".$Access."]";
	if ($Access == "FrAc" )
	{
		...
		$t = new eZTemplate( "eztransaccion/user/" . $ini->read_var( "eZTransaccionMain", "TemplateDir" ),
							 "eztransaccion/user/intl/", $Language, "tarjprepconsalmov.php" );

		$t->setAllStrings();
		 
		$t->set_file( array(
			"tarjprepconsalmov_tpl" => "tarjprepconsalmov.tpl"
			) );
			
		$t->set_var( "transaccion_buffer", $transaccion_buffer );

		$t->pparse( "output", "tarjprepconsalmov_tpl" );
		
		
		$result = ControlAcceso("tarjprep",$usr);
		if($result=="0000")
			include_once("eztransaccion/user/include/tarjprepconsalmov_ajax.inc");
		else
			echo $result;
