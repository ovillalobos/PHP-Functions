...
if ( $user )
{
    $t = new eZTemplate( "eztransaccion/user/" . $ini->read_var( "eZTransaccionMain", "TemplateDir" ),
                         "eztransaccion/user/intl/", $Language, "ecdcambioemail.php" );

    $t->setAllStrings();

    $t->set_file( array(
        "ecdcambioemail_tpl" => "ecdcambioemail.tpl"
        ) );
    ...
    $transaccion_buffer = "";
    $Empresa = "3"; //-- EVG-EDS Bajionet CNBV 01042005 agregue campo Empresa
...
//  print($_POST['Access']);
    switch($_POST['Access']) {
    case "FrAc":
        $particularFields = "&Estado=".$Estado."&Empresa=".urlencode($Empresa)."&AgruPagIni=".urlencode($AgruPagIni)."&FrAccount=".urlencode($_POST['FrAccount'])."&Locali=".$_POST['Locali']."&DireccB=".$_POST['DireccB']."&emailej=".urlencode( $emailej )."&Button=".urlencode( $_POST['Button'] );
        $tr->blog($qki,"Envio",$Empresa,$AgrupaPagIni,$_POST['FrAccount'],$_POST['Locali'],$emailej,$Estado,$_POST['DireccB'],$_POST['Access'] );
    //    print($particularFields);
        break;
   case "Confirm":
        $particularFields = "&Estado=".$Estado."&Empresa=".urlencode($Empresa)."&AgruPagIni=".urlencode($AgruPagIni)."&FrAccount=".urlencode($_POST['FrAccount'])."&Locali=".$_POST['Locali']."&DireccB=".$_POST['DireccB']."&emailej=".urlencode( $emailej )."&Button=".urlencode( $_POST['Button'] );
        $tr->blog( $qki, "Envio", $Empresa,$AgrupaPagIni, $_POST['FrAccount'], $_POST['Locali'],$emailej,$Dias,$_POST['DireccB'],$Accion );
    //    print($particularFields);
        break;
    case "Process":
         $particularFields = "&Passwd=".encrypt( $_POST['code'], strtolower( $usr ) )."&Estado=".$Estado."&Empresa=".urlencode($Empresa)."&AgruPagIni=".urlencode($AgruPagIni)."&FrAccount=".urlencode($_POST['FrAccount'])."&Locali=".$_POST['Locali']."&DireccB=".$_POST['DireccB']."&emailej=".urlencode( $emailej )."&Button=".urlencode( $_POST['Button'] );
        $tr->blog( $qki, "Envio", $Empresa,$AgrupaPagIni, $_POST['FrAccount'], $_POST['Locali'],$emailej,$Dias,$_POST['DireccB'],$Accion );
    //    print($particularFields);
        break;
	...
	$t->pparse( "output", "ecdcambioemail_tpl" );
...
else
{
	...
	$t->set_var( "redirect_url", "/transaccion/ecdcambioemail/" );
