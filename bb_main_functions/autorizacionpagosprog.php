...
if ( $user )
{
    ...
    $t = new eZTemplate( "eztransaccion/user/" . $ini->read_var( "eZTransaccionMain", "TemplateDir" ),
                         "eztransaccion/user/intl/", $Language, "AutorizacionPagosProg.php" );

    $t->setAllStrings();

    $t->set_file( array(
        "AutorizacionPagosProg_tpl" => "AutorizacionPagosProg.tpl"
        ) );
    ...
    $t->pparse( "output", "AutorizacionPagosProg_tpl" );

    $usr = strtolower($usr);   

    $result = ControlAcceso("autpagprog",$usr);

    if($result=="0000")
    	include("eztransaccion/user/include/pagosprogporautorizar.inc");
    else
        echo $result;
