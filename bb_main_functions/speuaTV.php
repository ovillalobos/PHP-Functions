<?php
//
// $Id: speuaTV.php,v 1.0 2001/11/01 11:22:30 smb Exp $
//
...
if ( $user )
{
    $t = new eZTemplate( "eztransaccion/user/" . $ini->read_var( "eZTransaccionMain", "TemplateDir" ),
                         "eztransaccion/user/intl/", $Language, "speuaTV.php" );

    $t->setAllStrings();

    $t->set_file( array(
        "speuaTV_tpl" => "speuaTV.tpl"
        ) );
...
//HB    
    ...
    case "Confirm":
    case "Process":
        $particularFields = "&FrAccount=".urlencode($FrAccount)."&Apocope=".urlencode($Apocope)."&Amount=".urlencode($Amount)."&ToAccount=".urlencode($ToAccount)."&NombreB=".urlencode($NombreB)."&DireccB=".urlencode($DireccB)."&LadaB=".urlencode($LadaB)."&TeleB=".urlencode($TeleB)."&Desc=".urlencode(preg_replace("/&/", "-", $Desc))."&RFC=".urlencode($RFC)."&RenInt=".urlencode($RenInt)."&Button=".urlencode($Button)."&Empresa=".urlencode($Empresa);  /* HB AGL*/
        $tr->blog($qki,"SpeuaTV", $FrAccount, $Apocope, $PlazaB, $NombreB, $Amount);
        break;
    }
    $transaccion_buffer = "";
    // DebugBreak();
    $ret_code = $tr->PostToHost($backend, "/IBnkIIS.dll", "Trxn=spi&Access=".urlencode($Access)."&CustID=".urlencode($usr)."&Cadpriv=".urlencode($priv).$particularFields, $qki, $usr, $qki, $priv, $transaccion_buffer); // SPEUATV

    $t->set_var( "transaccion_buffer", $transaccion_buffer );
    $t->pparse( "output", "speuaTV_tpl" );
...
else
{
    ...
    $t->set_var( "redirect_url", "/transaccion/speuaTV/" );
