...
function modSubmit ($submit)
{
	...
	$submit=str_replace("ONCLICK=\"parent.history.back()\"","ONCLICK=\"iniProcesarMenu('pagosanuales', '')\"",$submit);
...
$procesarAjax="<script>
						...
						function procesarAjax()
						{
							...
							{
								document.frmApplet.Datos1.value= document.Ejercicio.appEnviaDatos();
								document.frmApplet.DatosImp.value=document.Ejercicio.appEnviaImpresion();
								document.frmApplet.CadOrig.value=document.Ejercicio.appEnviaCadOriginal();
								...
								if (document.frmApplet.Datos1.value == 'MSG|')
								{
									window.alert('Debe cerrar la ventana de dialogo.');
									return false;
								}
								dImp=Acentos(dImp);
							}
							...
							iniProcesarMenu('pagosanuales', parametros);
						}
				</script>";
...
$DomainPostfix = $ini->read_var( "site", "DomainPostfix" );
    ...
    $noPagProv=0;    
    $fechaLimite='20150918';
   
   if ($fecActual >= $fechaLimite)
    {
	$mensaje_sat="	 <table width='80%' height='62' border='1' align='center' cellpadding='5' cellspacing='0'><tr><td colspan='0' align='center'> <font size=5 color=red> <ins> <b> AVISO </b></ins></font> <br> El Servicio de Administración Tributaria informa:<br>Esta aplicaci&oacute;n dejar&aacute; de funcionar el 18 de septiembre de 2015, ya que a partir de agosto de 2015 los contribuyentes deben utilizar el Servicio de Pago Referenciado para presentar sus declaraciones anuales de impuestos federales.</td></tr></table>";
	$noPagProv=1;    
    }
	...
	{	
		$mensaje_sat="	 <table width='80%' height='62' border='1' align='center' cellpadding='5' cellspacing='0'><tr><td colspan='0' align='center'> <font size=5 color=red> <ins> <b> AVISO </b></ins></font> <br> El Servicio de Administración Tributaria informa:<br>Esta aplicaci&oacute;n dejar&aacute; de funcionar el 18 de septiembre de 2015, ya que a partir de agosto de 2015 los contribuyentes deben utilizar el Servicio de Pago Referenciado para presentar sus declaraciones anuales de impuestos federales.</td></tr></table>";
		//$transaccion_buffer= $mensaje_sat.$transaccion_buffer;
	}
    }

    
	$t->set_var( "transaccion_buffer", $mensaje_sat.$transaccion_buffer.$Pagina );  //ATAR MSJ BLOQUEO IMPTOS 06AGO2015
	$t->pparse( "output", "pagosanuales_tpl" );
	
	//$t->set_var( "transaccion_buffer", $transaccion_buffer.$Pagina );
	//$transaccion_buffer=$t->parse( "output", "pagosanuales_tpl" );
