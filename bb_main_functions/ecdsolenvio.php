...
if ( $user )
{
    $t = new eZTemplate( "eztransaccion/user/" . $ini->read_var( "eZTransaccionMain", "TemplateDir" ),
                         "eztransaccion/user/intl/", $Language, "ecdsolenvio.php" );

    $t->setAllStrings();

    $t->set_file( array(
        "ecdsolenvio_tpl" => "ecdsolenvio.tpl"
        ) );
...
//  print($_POST['Access']);
    switch($_POST['Access']) {
    case "FrAc":
        $particularFields = "&Estado=".$_POST['Estado']."&Empresa=".urlencode($Empresa)."&AgruPagIni=".urlencode($AgruPagIni)."&FrAccount=".urlencode($_POST['FrAccount'])."&Locali=".$_POST['Locali']."&DireccB=".$_POST['DireccB']."&emailej=".urlencode( $_POST['emailej'] )."&Button=".urlencode( $_POST['Button'] );
        $tr->blog($qki,"Envio",$Empresa,$AgrupaPagIni,$_POST['FrAccount'],$_POST['Locali'],$_POST['emailej'],$_POST['Estado'],$_POST['DireccB'],$_POST['Access'] );
       // print($particularFields);
        break;
   case "Confirm":
        $particularFields = "&Estado=".$_POST['Estado']."&Empresa=".urlencode($Empresa)."&AgruPagIni=".urlencode($AgruPagIni)."&FrAccount=".urlencode($_POST['FrAccount'])."&Locali=".$_POST['Locali']."&DireccB=".$_POST['DireccB']."&emailej=".urlencode( $_POST['emailej'] )."&Button=".urlencode( $_POST['Button'] );
        $tr->blog( $qki, "Envio", $Empresa,$AgrupaPagIni, $_POST['FrAccount'], $_POST['Locali'],$_POST['emailej'],$_POST['Dias'],$_POST['DireccB'],$Accion );
       // print($particularFields);
        break;
    case "Process":
         $particularFields = "&Passwd=".encrypt( $_POST['code'], strtolower( $usr ) )."&Estado=".$_POST['Estado']."&Empresa=".urlencode($Empresa)."&AgruPagIni=".urlencode($AgruPagIni)."&FrAccount=".urlencode($_POST['FrAccount'])."&Locali=".$_POST['Locali']."&DireccB=".$_POST['DireccB']."&emailej=".urlencode( $_POST['emailej'] )."&Button=".urlencode( $_POST['Button'] );
        $tr->blog( $qki, "Envio", $Empresa,$AgrupaPagIni, $_POST['FrAccount'], $_POST['Locali'],$_POST['emailej'],$_POST['Dias'],$_POST['DireccB'],$Accion );
        $Proc = 1;
       // print($particularFields);
        break;
    ...
    $transaccion_buffer = "";
    // DebugBreak();
    $ret_code = $tr->PostToHost($backend, "/IBnkIIS.dll", "Trxn=ecd&Access=".urlencode($_POST['Access'])."&CustID=".urlencode($usr)."&Cadpriv=".urlencode($priv).$particularFields, $qki, $usr, $qki, $priv, $transaccion_buffer); // tarjeta

    if ( $ret_code == 0 && $Proc == 1 )
    {
    	$session->setVariable( "r_ecdsol", trim( 1 ) );
    }


    $t->set_var( "transaccion_buffer", $transaccion_buffer );
    $t->pparse( "output", "ecdsolenvio_tpl" );
...
else
{
	...
	$t->set_var( "redirect_url", "/transaccion/ecdsolenvio/" );
