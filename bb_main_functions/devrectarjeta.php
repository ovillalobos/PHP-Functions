...
//echo "[".$Access."]";
	if ($Access == "FrAc" )
	{
		...
		$t = new eZTemplate( "eztransaccion/user/" . $ini->read_var( "eZTransaccionMain", "TemplateDir" ),
							 "eztransaccion/user/intl/", $Language, "devrectarjeta.php" );

		$t->setAllStrings();
		 
		$t->set_file( array(
			"devrectarjeta_tpl" => "devrectarjeta.tpl"
			) );
			
		$t->set_var( "transaccion_buffer", $transaccion_buffer );

		$t->pparse( "output", "devrectarjeta_tpl" );
		
		$result = ControlAcceso("tarjprep",$usr);
		if($result=="0000")
			include_once("eztransaccion/user/include/devrectarjeta.inc");
		else
			echo $result;
...
else
{
	...
	$t->set_var( "redirect_url", "/transaccion/devrectarjeta/" );
