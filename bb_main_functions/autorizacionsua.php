...
if ( $user )
{
    $t = new eZTemplate( "eztransaccion/user/" . $ini->read_var( "eZTransaccionMain", "TemplateDir" ),
                         "eztransaccion/user/intl/", $Language, "autorizacionsua.php" );
...
//	echo "YEHO valiables ANTES ASIG [$rpatronal] [$foliosua] [$perpago] [$_POST['Access']]";
...
//	echo "YEHO valiables DESPUES ASIG [$rpatronal] [$foliosua] [$perpago] [$_POST['Access']]";

    switch($_POST['Access']) {
    case "FrAc":
	//echo "YEHO ESTOY EN SUA FRACCS";
	 if ( $_POST['Agrupa']=="Aceptar" )
	 {
	 	...
	 	$Accion  = "autorizasua";
        	...
        	$tr->blog($qki,"Autorizacion", $_POST['FrAccount'], $Nomben1, $Desc, $Amount, $Accion);
	 }
	 else
	 {
		$particularFields = "";
        	$tr->blog($qki,"Autorizaciones", $_POST['FrAccount'], $Nomben1, $Desc, $Amount, $Accion);
	 }
        break;
    case "Confirm":
   // 	echo "YEHO ESTOY EN SUA CONFIRM acion[$Accion] nomben1[$Nomben1]";
...
//		echo "YEHO Guardo el fraccount[$fraccount]";
        $particularFields = "&Access=".urlencode($_POST['Access'])."&Nomben1=".urlencode($Nomben1)."&Desc=".urlencode($Desc)."&FrAccount=".urlencode($_POST['FrAccount']);;
        $tr->blog($qki,"Autorizaciones", $_POST['FrAccount'], $Nomben1, $Desc, $Amount, $Accion);
        break;
    case "Process":
//   	echo "YEHO ESTOY EN SUA PROCESS";
...
//		echo "YEHO fraccount [$fraccount]";
        //$particularFields = "&Passwd=".encrypt( $_POST['code'], strtolower( $usr ) )."&Access=".urlencode($_POST['Access'])."&Nomben1=".urlencode($Nomben1)."&Desc=".urlencode($Desc)."&FrAccount=".urlencode($_POST['FrAccount']);;
		$particularFields = "&Passwd=".encrypt( $_POST['code'], strtolower( $usr ) )."&Access=".urlencode($_POST['Access'])."&Nomben1=".urlencode($Nomben1)."&Desc=".urlencode($Desc)."&FrAccount=".urlencode($_POST['FrAccount'])."&Nomben3=".urlencode($Nomben3);;  //YEHO PARA MANDAR LOS CAMPOS DE SUA
        $tr->blog($qki,"Autorizaciones", $_POST['FrAccount'], $Nomben1, $Desc, $Amount, $Accion);
    ...
    $ret_code = $tr->PostToHost($backend, "/IBnkIIS.dll", "Trxn=aus&Access=".urlencode($_POST['Access'])."&CustID=".urlencode($usr)."&Cadpriv=".urlencode($priv).$particularFields, $qki, $usr, $qki, $priv, $transaccion_buffer); // autorizacion de operaciones pendientes
...
else
{
    ...
    $t->set_var( "redirect_url", "/transaccion/autorizacionsua/" );
