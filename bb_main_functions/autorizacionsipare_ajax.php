...
include_once( "ezreimp/classes/ezreimp.php" ); //DBA Reimpresion de Comprobantes
	...
	function modSubmit ($submit)
	{
		...
		$submit=str_replace("parent.history.back()","iniProcesarMenu('autorizacionsipare', '');",$submit);
		return $submit;
	}
    ...
    switch($Access) {
    case "FrAc":
	//echo "HOLA JEJEJESUA Nomben1[$Nomben1]";
	 if ( $Agrupa=="Aceptar" )
	 {
	 	$Access = "FrAc";
	 ...
	 {
		$particularFields = "";
	//	echo "YEHOelse accion[$Accion]";
        	$tr->blog($qki,"Autorizaciones", $FrAccount, $Nomben1, $Desc, $Amount, $Accion);
	 }
        break;
    case "Confirm":
   // echo "SUA COnfirm Nomben1[$Nomben1]";
        $particularFields = "&Access=".urlencode($Access)."&Nomben1=".urlencode($Nomben1)."&Desc=".urlencode($Desc)."&FrAccount=".urlencode($FrAccount);;
        //$particularFields = "&Access=".urlencode($Access)."&Nomben1=".urlencode($Nomben1)."&Desc=".urlencode($Desc)."&FrAccount=".urlencode($FrAccount)."&Accion=".urlencode($Accion);;
    //    echo "YEHO autconfirm Accion [$Accion] pariculars[$particularFields]";
        $tr->blog($qki,"Autorizaciones", $FrAccount, $Nomben1, $Desc, $Amount, $Accion);
        break;
    case "Process":
       ...
       // echo "YEHO autprocess Accion [$Accion] pariculars[$particularFields]";
        $tr->blog($qki,"Autorizaciones", $FrAccount, $Nomben1, $Desc, $Amount, $Accion);
        break;
    }
    $transaccion_buffer = "";
...
//	echo "YEHO autposttohost Accion[$Accion] Access[$Access] particularFields[$particularFields]";
    $ret_code = $tr->PostToHost($backend, "/IBnkIIS.dll", "Trxn=als&Access=".urlencode($Access)."&CustID=".urlencode($usr)."&Cadpriv=".urlencode($priv).$particularFields, $qki, $usr, $qki, $priv, $transaccion_buffer); // autorizacion de operaciones pendientes
//////////////////// YEHO I 13Abr2011
if ($Access == "Process")
	{
