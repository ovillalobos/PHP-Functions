...
include("eztransaccion/user/include/controles_javascript.inc"); // RAI 10Feb2012 - Track 185966 - Agrupación pemex-gas
...
if ( $user )
{
    $t = new eZTemplate( "eztransaccion/user/" . $ini->read_var( "eZTransaccionMain", "TemplateDir" ),
                         "eztransaccion/user/intl/", $Language, "PmxDocPPag.php" );

    $t->setAllStrings();

    $t->set_file( array(
        "PmxDocPPag_tpl" => "PmxDocPPag.tpl"
        ) );
...
//HB    
    ...
    case "FrAc":
        $particularFields = "";
        $_POST['Accion'] = "ListDPP";
		// RAI 29Feb2012 - Inicio - Track 185966 - Agrupación pemex-gas
		if ($_POST['AgruPagIni'] == 1 )
		{
			...
			$session->setVariable( "pmx_custno", $FrAccount );
		...
		// RAI 29Feb2012 - Fin - Track 185966 - Agrupación pemex-gas
    ...
    case "Confirm":
		...
		$gene = $session->variable( "pmx_custno" ); // RAI 29Feb2012 - Track 185966 - Agrupación pemex-gas
	...
	{
		//echo "Token: '".$token."'";
		$ret_code = $tr->PostToHost($bkend, "/IBnkIIS.dll", "Trxn=ctk&CustTkn=".encrypt( $token, strtolower( $usr ) )."&Access=".urlencode("process")."&CustID=".urlencode($usr)."&Cadpriv=".urlencode($priv) , $qki, $usr, $qki, $priv, $transaccion_buffer);

		if (strpos($transaccion_buffer,"TKNRESPTRUE")===false)
		{
			$t->set_var( "transaccion_buffer", $transaccion_buffer );
			$t->pparse( "output", "PmxDocPPag_tpl" );
			return;
		}
	}
...
else
{
    ...
    $t->set_var( "redirect_url", "/transaccion/PmxDocPPag/" );
